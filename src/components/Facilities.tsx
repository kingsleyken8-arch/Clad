...
const KIDS_IMAGE = `${CDN}/hf_20260614_143448_dfa29760-0e4a-4242-9b5c-575ae35edcea.png`;
...
function BracketIcon({ icon: Icon }: { icon: LucideIcon }) {
  return (
    <div className="relative mx-auto flex h-28 w-28 items-center justify-center">
      ...
      <Icon className="h-11 w-11 text-orange-500" strokeWidth={1.25} />
    </div>
  );
}
...
function PromoPanel({ title, subtitle, image, alt }: PromoProps) {
  return (
    <div className="relative flex min-h-[320px] items-center overflow-hidden bg-neutral-100 lg:min-h-[380px]">
      {/* Image bleeds in from the right */}
      <img
        src={image}
        alt={alt}
        loading="lazy"
      ...
      />
      <div className="absolute right-0 top-0 h-full w-3/5 bg-gradient-to-r from-neutral-100 via-neutral-100/40 to-transparent" />

      {/* Copy */}
      <div className="relative z-10 max-w-[60%] px-8 py-12 sm:px-12 lg:px-16">
        <h3 className="font-inter text-3xl font-extrabold uppercase tracking-tight text-neutral-900 sm:text-4xl">
          {title}
        </h3>
        <p className="mt-3 font-inter text-sm text-neutral-500">{subtitle}</p>
        <a
          href="#"
          className="mt-6 inline-flex items-center border-2 border-orange-500 px-7 py-3 font-inter text-[11px] font-semibold uppercase tracking-widest text-neutral-900 transition-colors hover:bg-orange-500 hover:text-white"
        >
          Shop Now
        </a>
      </div>
    ...
    <section id="facilities" className="w-full bg-white text-neutral-900">
      ...
      <div className="mx-auto max-w-7xl px-6 py-24 sm:px-10 lg:px-16 lg:py-28">
        <div className="text-center">
          <Reveal>
            <span className="font-inter text-xs font-semibold uppercase tracking-[0.3em] text-orange-500">
              Your Ride Starts Here.
            </span>
          </Reveal>
          <Reveal delay={100}>
            <h2 className="mt-4 font-inter text-4xl font-extrabold tracking-tight text-neutral-900 sm:text-5xl lg:text-6xl">
        ...
        <div className="mt-16 grid grid-cols-1 gap-12 md:grid-cols-3 md:gap-0">
          {FEATURES.map((feature, i) => (
            ...
            >
              ...
              <h3 className="mt-7 font-inter text-lg font-bold uppercase tracking-wide text-neutral-900">
                {feature.title}
              </h3>
